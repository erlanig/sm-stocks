...
def run_monte_carlo(prices, days, simulations):
    ...
    if len(np.unique(final_prices)) == 1:
        ...
        final_prices = final_prices + np.random.normal(0, final_prices[0] * 0.0001, simulations)
        prob_up = max(0.1, min(99.9, float(np.mean(final_prices > last_price) * 100)))
...
def make_chart(price_paths, stats, paths_display, ticker, days, simulations, mode='standard'):
    ...
    prob_down = stats.get('prob_down', 50.0)
    if np.isnan(prob_down) or not np.isfinite(prob_down):
        prob_down = 50.0
    
    # Ensure probabilities sum to 100
    if abs(prob_up + prob_down - 100) > 0.01:
        prob_up = 50.0
        prob_down = 50.0
    ...
    # Safe pie chart creation with validation
    colors_pie = [P['green'], P['red']]
    
    # Ensure values are valid for pie chart
    pie_values = [prob_up, prob_down]
    
    # Filter out any NaN or invalid values
    pie_values = [v if np.isfinite(v) else 0.0 for v in pie_values]
    
    # If both are zero, set defaults
    if sum(pie_values) == 0:
        pie_values = [50.0, 50.0]
    ...
    try:
        ...
        # Fallback: create a simple circle with text
        circle = plt.Circle((0, 0), 0.55, color=P['border'], fill=True, 
                           edgecolor=P['bg'], linewidth=3)
    ...
    # Center text
    ax2.text(0, 0.18, f"{prob_up:.1f}%", ha='center', va='center',
             color=P['green'] if prob_up >= 50 else P['red'],
             fontsize=22, fontweight='800')
    ax2.text(0, -0.10, 'Probabilitas Naik', ha='center', va='center',
             color=P['mid'], fontsize=9, fontweight='600')
    ax2.text(0, -0.38, f"{prob_down:.1f}% Probabilitas Turun",
